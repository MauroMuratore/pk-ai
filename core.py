    ...
    def fit(self, env, nb_steps, action_repetition=1, callbacks=None, verbose=1,
        ...
        try:
            while self.step < nb_steps:
                print(f"core 124----------------------step{self.step}----------------------")
                if observation is None:  # start of a new episode
                    ...
                    for _ in range(nb_random_start_steps):
                        ...
                        if self.processor is not None:
                            print("Core 151 done->terminated, truncated")
                            observation, reward, terminated, truncated, info = self.processor.process_step(observation, reward, done, info)
                            done = terminated or truncated
                ...
                print(f"core 169 {observation.shape}")
                ...
                for _ in range(action_repetition):
                    if done:
                        ...
                        break
                    callbacks.on_action_begin(action)
                    print("Core 178 done -> terminated, truncated")
        ...
        for episode in range(nb_episodes):
            ...
            for _ in range(nb_random_start_steps):
                ...
                if self.processor is not None:
                    action = self.processor.process_action(action)
                callbacks.on_action_begin(action)
                print("CORE 330 done -> terminated")
                observation, r, terminated, truncated, info = env.step(action)
                done = terminated or truncated
                observation = deepcopy(observation)
                if self.processor is not None:
            ...
            while not done:
                ...
                for _ in range(action_repetition):
                    callbacks.on_action_begin(action)
                    print("CORE 356 done -> terminated, truncated")
                    observation, r, te,tr, info = env.step(action)
                    d = te or tr
                    observation = deepcopy(observation)
                    if self.processor is not None:
